...
现在的Agent以及完全可以编写好驱动层的库函数了，在古法编程就有点招笑了，所以这个项目也就不会再上传手搓的库文件了。之后所有的更新都将是AI生成再经过整理后的库了。未来工程师可能更应该注重与项目架构的管理而非具体实现的细节。
——26.04.21
...
| 组件名        | 完成时间   | 进度   | 备注                       |
| ------------- | ---------- | ------ | -------------------------- |
| Clock         | 2024/10/14 | 已完成 | 时钟相关库                 |
| OLEDGLib      | 2024/10/14 | 已完成 | OLED图形显示库             |
| CRTP          | 2024/10/29 | 已完成 | 只支持在FreeRTOS环境下     |
| Shell         | 2025/05/11 | 已完成 |                            |
| FatFs文件系统 | 2025/06/23 | 已完成 | 仅大容量型号可用，来源反客 |
